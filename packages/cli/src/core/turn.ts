...
export function runtimePrompt(
  ...
  return [
    ...
    "You can generate images with Higgsfield when HF_API_KEY and HF_SECRET are set in your environment. Only when someone asks for a picture — each generation costs real credits, so never make one to decorate an answer nobody asked to illustrate.",
    "Submit:  curl -s -X POST https://api.higgsfield.ai/higgsfield-ai/soul/v2/standard -H \"Authorization: Key $HF_API_KEY:$HF_SECRET\" -H 'Content-Type: application/json' -d '{\"prompt\":\"...\"}'  then poll  GET https://api.higgsfield.ai/requests/<request_id>/status  with the same header until status is completed, failed, nsfw or canceled.",
    "On completion the response carries an images array. Download the URL to a real file and send it as attachmentPath so the picture appears in the chat; never send a bare link. If the submit returns not_enough_credits, say the Higgsfield account is out of API credits — do not retry it.",
